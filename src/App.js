...
function UserSetup({ setIsLoggedIn, setIsSetupComplete }) {
  ...
  const likeDislikeOptions = ['Food to mankind', 'Social networking', 'Healthy eating', 'Fast food', 'Outdoor activities', 'Indoor games'];
...
function ProfilePage() {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        const data = { name: 'John Doe', age: 30, email: 'john@example.com', dietaryRestrictions: 'None' };
        setProfile(data);
        setName(data.name);
        setAge(data.age);
        setEmail(data.email);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const updatedProfile = { name, age, email, dietaryRestrictions };
    try {
      ...
      {isEditing ? (
        <form onSubmit={handleSubmit} className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
          ...
          <div className="mb-3">
            ...
            <textarea className="form-control" id="dietaryRestrictions" value={dietaryRestrictions} onChange={(e) => setDietaryRestrictions(e.target.value)} />
          </div>
        ...
        <div className="card shadow-sm">
          <div className="card-body">
            ...
            <p className="card-text">Dietary Restrictions: {profile?.dietaryRestrictions}</p>
    ...
    <div className="scan pt-16 container mx-auto px-4 text-center">
      ...
      <div className="card shadow-sm mx-auto" style={{ maxWidth: '500px' }}>
        <div className="card-body">
          ...
          {image && <img src={image} alt="Preview" className="img-fluid mb-3" style={{ maxHeight: '300px' }} />}
          <button onClick={handleScan} className="btn btn-primary" disabled={isUploading}>
